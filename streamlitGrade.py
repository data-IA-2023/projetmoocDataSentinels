...
def grade_prediction(dict):
    ...
    course = dict["new_mooc"]
    bodyemotion = dict["new_emotion"]
    country = dict["new_mooc"]
    gender = dict["new_genre"]
    year_of_birth = dict["new_age"]
    csp = dict["new_csp"]
    level_of_education = dict["new_level_educ"]
    ...
    if prediction < 0.6 :
        st.error(f"La prédiction de la note est : {prediction}, donc peut de chance d'obtenir son diplome")
    if prediction >= 0.6 :
        st.success(f"La prédiction de la note est : {prediction}, donc de grande chance d'obtenir son diplome")
